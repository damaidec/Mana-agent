...
PVOID findNtDll(void) {
    ...
    PMANA_PEB pPeb = NULL;
    PMANA_PEB_LDR_DATA pLdrData = NULL;
    PMANA_LDR_DATA_TABLE_ENTRY pModuleEntry = NULL, pModuleStart = NULL;
    PUNICODE_STR pDllName = NULL;

    PVOID pNtdllBase = NULL;

    pPeb = (PMANA_PEB)__readgsqword(0x60);
    pLdrData = pPeb->pLdr;
    pModuleEntry = pModuleStart = (PMANA_LDR_DATA_TABLE_ENTRY)pLdrData->InMemoryOrderModuleList.Flink;
    ...
    do {
        ...
        if (djb2_unicode(toLower(pDllName->pBuffer)) == xor_hash(H_MODULE_NTDLL)) {
            pNtdllBase = (PVOID)pModuleEntry->DllBase;
            break;
        }

        pModuleEntry = (PMANA_LDR_DATA_TABLE_ENTRY)pModuleEntry->InMemoryOrderModuleList.Flink;
...
unsigned long djb2_unicode(const wchar_t* str) {
    unsigned long hash = APIhashval2;
    DWORD val;
